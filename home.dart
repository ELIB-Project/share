...
class _ToolScoreBoxState extends State<ToolScoreBox> with TickerProviderStateMixin{
    ...
    percentageAnimationController =  AnimationController(
      ...
      ..addListener((){
        setState(() {
          ...
          gapPercentage=lerpDouble(gapPercentage,newgapPercentage,percentageAnimationController.value)!;
        });
      });
      ...
      setState(() {
        ...
        double gap = 0.0;
        if(widget.toolScore>widget.oldToolScore) {gap = widget.toolScore.toDouble() - widget.oldToolScore.toDouble();}
        else {gap = widget.oldToolScore.toDouble() - widget.toolScore.toDouble();}
...
class _TrainingScoreBoxState extends State<TrainingScoreBox> with TickerProviderStateMixin{
  double percentage = 0.0;
  double newPercentage = 0.0;
  double gapPercentage = 0.0;
  double newgapPercentage = 0.0;
  ...
  void initState() {
    ...
    percentageAnimationController =  AnimationController(
      ...
      setState(() {
        ...
        double gap = 0.0;
        if(widget.trainScore>widget.oldTrainScore) {gap = widget.trainScore.toDouble() - widget.oldTrainScore.toDouble();}
        else {gap = widget.oldTrainScore.toDouble() - widget.trainScore.toDouble();}
        gapPercentage = newgapPercentage;
        newgapPercentage = gap / 100;
        percentageAnimationController.forward();
...
class _SafetyScoreBoxState extends State<SafetyScoreBox> with TickerProviderStateMixin {
  double percentage = 0.0;
  double newPercentage = 0.0;
                ...
                Column(
                  ...
                  children: [
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        PercentScore(percent: percentage, color: Color(safetyColor), fontSize: 80.0, width: 100.0),
                      ],
                    ),   
                  ],
                ),
...
class PercentScore extends StatelessWidget {
  const PercentScore({Key? key, required this.percent, required this.color, required this.fontSize, required this.width})
      : super(key: key);
  final percent;
  final color;
  final fontSize;
...
class PercentScorePaint extends CustomPainter {
  ...
  Color activeColor;
  double fontSize;
  PercentScorePaint({required this.percentage, required this.activeColor, required this.fontSize});
